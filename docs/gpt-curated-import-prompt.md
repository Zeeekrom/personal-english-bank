# GPT Curated Import Prompt

Use this prompt when one completed source needs to be converted into a Personal
English Bank Curated Import Package.

1. Send only one source file per GPT conversation/run.
2. Replace the five placeholders at the end of the prompt.
3. Save GPT's reply as UTF-8 JSON, or paste it directly into the application's
   `/import` page.
4. Do not add Markdown fences around GPT's reply.

## Copy-ready prompt

```text
你是 Personal English Bank 的外部语料整理器。请把我随后提供的一个来源文件整理为可直接导入系统的 Curated Import Package。
